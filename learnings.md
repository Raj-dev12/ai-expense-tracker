...
**Ground truth**
What the thing being measured actually says, written down by a person before any measuring
starts. For the receipts it is the shop name, the date and the total, typed in by hand off the
paper. It matters because every other number available here can be influenced by the change
being tested: the OCR engine's confidence rises whenever an image is sharpened, whether or not
the letters it picked were right, so judging a sharpening step by that score asks the step to
mark its own homework. Ground truth is the only input in the exercise that no amount of image
processing can move.

**Measuring with the ruler you are adjusting**
The trap ground truth avoids. If a process picks whichever option scores best on some measure,
that measure can no longer tell you whether the process improved — it can only go up, because
going up is how the option was chosen. The receipt code is about to use the arithmetic checks to
*choose* between two prepared images, which is a good use of them; that is exactly why the
checks cannot also be the scoreboard. Two jobs, two instruments.

**Baseline**
The measurement taken *before* a change, so the change has something to be compared against.
Easy to skip and impossible to reconstruct afterwards: once the code has changed there is no
way to find out what the old version would have done on those photos, short of putting the old
version back. Hence the comparison bench exists before either new technique does.

**Regression, and a regression guard**
A regression is something that used to work and stopped. A regression guard is a case kept
around specifically to notice one — here, the `flat` bucket of receipts that read correctly
today. Their job is not to get better. It is to fail loudly if a change that helps the hard
photos quietly ruins the easy ones, which is a real risk when a step replaces information
rather than adding to it.

**Entry point**
The file a build starts from. Vite treats every HTML file in the project root as one while
developing, and builds only `index.html`, which is why `compare.html` is reachable at
`/compare.html` on the dev server and is absent from a production build. That is the mechanism
keeping a page that loads real receipt photos out of anything deployed — not a rule somebody has
to remember, but the default behaviour of the tool.

**Negative case**
A test whose correct answer is "nothing". Most cases check that the right value comes out; a
negative case checks that a wrong value does *not* get invented when there is nothing to find.
They are easy to leave out, because a thing that is absent does not suggest a test the way a
thing that is present does — and they catch a whole class of bug that positive cases cannot see,
since a correct answer sitting in the way hides an invented one. The receipts with no date
printed on them are ten negative cases that arrived by accident.
---
...
| One function, routed by a rewrite rather than by its filename | `api/index.ts` plus `"/api/(.*)" -> "/api/index"` in `vercel.json`, after `api/[...path].ts` failed | Catch-all brackets are a Next.js convention, not a Vercel one. Plain function routing read `[...path]` as a single dynamic segment literally named `...path`, so one-segment paths worked by accident, `/api/analytics/summary` got Vercel's own 404 without ever reaching Fastify, and every request arrived with a stray `...path` query parameter that the strict filter schemas correctly refused. Found by probing seven paths on the live deployment rather than by trusting that the first one answering meant routing worked. |
| A pie slice carries the categories it stands for | `PieSlice.members`, and the click filters on that set | The fold collapses everything past the fifth category into one slice, and that slice carried only the name "Other" — which is also a real category. So the tooltip described five categories summed together while the panel below it listed the one. Both halves were correct in isolation; the bug lived between them, which is why the existing checks all passed. A slice now knows what it is made of and everything that reads a slice reads the same list. |
| The category filter takes several | `?category=A&category=B`, repeated key rather than a comma-separated string | Category names are free text, so a delimiter inside one would split it in half. A repeated key is the ordinary HTTP idiom and Fastify hands it over as an array. Doing it in the backend rather than filtering in the browser keeps one definition of what "in these categories" means — the alternative would have put the rule in a second place, which is what the bug was. |
| A folded slice says it is a group and names its members | `Other · 5 categories`, with the names written out beneath | Without it a real category vanishes from the chart completely: Restaurants was drawn nowhere while the query box still answered questions about it by name, and nothing on screen said where it had gone. A chart that silently omits a category is worse than one that admits it grouped some together. |
| A comparison needs a baseline, not just a previous figure | Fewer than three expenses in the stretch before means no percentage, and the card says which silence it is | On the 1st of a month the period is one day and the stretch before it is one day, so a quiet day followed by a normal one reported "2586% more" — arithmetically right, and describing nothing but whether one purchase happened to land inside the window. Capping the number would have been the wrong fix: it would still be answering a question that should not have been asked. The test is about the baseline, so a genuinely large change against a real baseline is still reported. |
| "Nothing recorded" and "too little to compare" are different sentences | Three outcomes on the change card, not two | A reader who saw a comparison last week and a dash this week cannot tell whether the figure is missing or the spending is identical. Saying which costs one sentence and removes the guess. |
| Asks are counted, not just words | `asksIn()`, checked before a query shape is chosen | `unreadWords` guards against dropping a **constraint**; this guards against dropping an **ask**, which turned out to be a different hole in the same wall. "How much did I spend on restaurants today and where did I spend it" passed every existing check — every word known, category and date both matched, nothing unread — then answered the amount and dropped "where" in silence. The whitelist could not see it because "where" is a perfectly known word: it is not an unrecognised subject but an unrecognised ask. |
| A compound question is answered part by part | One query per ask, sharing the filters the whole sentence sets; anything with no shape is named in `unanswered` | It was first refused with both parts named, which was honest but weaker than it needed to be: both halves are answerable on their own, and only the *pair* was beyond one query. Running each ask as its own query and joining the sentences drops nothing and guesses at nothing. The filters are shared rather than re-read per clause because that is what the sentence means — in "how much did I spend on restaurants today and where did I spend it", the "it" is the restaurant spending of that day, and splitting the text into clauses would have widened the second part into a different question. |
| A compound holds only answerable parts | `parts: AnswerableQuestion[]`, a narrower union than `StructuredQuestion` | A compound containing a refusal, a signpost or another compound is not something to be careful about — it is something the type and the Zod schema have no way to express. The boundary stays a boundary. |
| The asks come out in sentence order | `asksIn` sorts by where each head appears | The answer is read beside the question. "When did I spend the most and where" answered with the shop first quietly reorders what was asked, and a reader has to check which sentence belongs to which half. |
| An unread constraint still beats splitting the question | The whitelist guard runs before the compound builder | Answering two halves of a question that was misread is worse than answering neither. A word nobody understood is also the more actionable complaint. |
| The period control is two controls | A dropdown for how long a block is, arrows for which block | Fourteen entries in one menu is not a menu. Separating the size of the window from which window it is makes "the July I mean" two clicks away, and keeps the list at seven names however far back somebody walks. |
| A stepped block is complete; the current one is not | August means 1–31 August; September means 1 September to today | There is nothing partial about a month that is over. It is also what makes the comparison honest — a whole August against a whole July is like for like, where "1 August to the 3rd" against a stretch of the same odd length compares one calendar accident with another. |
| Stepping stops at the present | The forward arrow is disabled at offset 0 | There is no spending in the future, and a control that walks into empty months invites the question of why they are empty. |
| Labels are absolute, never relative | "August 2026", not "last month" | "Last month" is readable exactly once; "three months ago" is arithmetic, and "three quarters ago" collides with the period actually called three quarters. An absolute name can be checked against a calendar — the same reason the comparison note names the window it compared against. |
| The picker always writes the dates out | "1 Sep – 3 Sep" under the control | Neither "This month" nor "August 2026" says where a partial block stops. Two dates mean nothing has to be inferred from a name. |
| A custom range gets no comparison | `compare=false`, and the card says "No comparison for a custom range" | "1 June to 15 July" has a perfectly well-defined stretch before it, with real spending in it — it is simply a stretch nobody chose, and a percentage against it invites a conclusion from an accident of arithmetic. Every named period has a natural predecessor; an arbitrary range does not. Passed by the caller rather than worked out from the dates, because "does this window have a natural predecessor" is a question about how it was chosen, and the dates no longer remember. |
...
| `sslmode` in the URL is refused outright | The app will not start if `DATABASE_URL` contains `sslmode`; `DB_SSL` is the only knob | They are two answers to one question and node-postgres does not merge them — the URL wins and the `ssl` option is ignored, so `?sslmode=require` turns `DB_SSL=require` into full verification, the one thing it was set to avoid. The failure then arrives as an unexplained certificate error from inside a TLS handshake. Silently stripping it would have been magic; letting it win would have been a trap. Refusing to start names the real knob. Found by inspecting what pg actually does with both set, before anyone tried it against a live database. |
| The baseline is measured before either technique is written | A comparison bench first, Sauvola and perspective correction second | There is no way to find out afterwards what the old preparation would have done on these photos. Skipping it is how two earlier rounds of preprocessing each felt like an improvement without anyone being able to say whether they were — recorded in this file at the time, and the same mistake was available again here. |
| Readings are scored against hand-typed ground truth, not the engine's confidence | The shop name, the date and the total, typed off the paper | Binarisation raises Tesseract's confidence *by construction* — the score measures how cleanly a shape matched the glyph it chose, and a hard black stroke matches some glyph more cleanly than a grey one, right or wrong. So for a change whose entire purpose is to sharpen the image, confidence is the one number guaranteed to approve of it. It is still shown on the bench, greyed out, because confidence rising while the scored columns stay flat is the signature of a cleaner-looking image that is still wrong. |
| The arithmetic verdict chooses; the typed labels measure | Two instruments, deliberately not one | `checks.ts` is a genuinely independent signal and is the right thing to pick a preparation with. That is exactly why it cannot also be the scoreboard: a measure that selects its own winner can only go up, and would report an improvement whether or not one happened. |
| The decisive count is silent wrong answers, not correct totals | Totals crossed with whether the checks flagged them | A step that gains two correct totals and adds one total that is wrong and uncontradicted has made the feature worse. The two correct ones were going to be confirmed by a person anyway; the wrong one is the plausible-looking value that gets scanned past, which is the failure this project has had four times now. |
| The bench also counts whether the truth was in the text at all | Loose digit and letter matching against the raw lines | The question the debug panel was built to answer, made countable. A total that was in the text and came out wrong is a parser problem; a total that was never in the text is a photo problem. Preprocessing can only move the second, so a grid that cannot tell them apart would credit a preparation step with fixes that belong to the parser. |
| The bench is a browser page, not a Node check | `compare.html`, alongside the jsdom checks rather than among them | Everything under test is canvas pixel work — `getImageData`, resampling, thresholds — and jsdom has no canvas. Stubbing one would mean measuring the stub. It also runs the code in the only place the code will ever actually run. |
| The fixture photos are outside the repository and outside the build | `receipt-fixtures/`, gitignored; the bench reads them through the file picker | They are somebody's real receipts with real shop names and amounts on them. `frontend/public/` was the obvious place and is wrong: anything there is served in development *and* copied into a production build, so one stray photo would be published. Vite builds only `index.html`, so the bench cannot be deployed either — the protection is the tool's default behaviour rather than a rule to remember. |
| Fixtures are grouped by filename, and a misnamed one goes to `unsorted` | `flat`, `dim`, `angled`, `creased` | The four groups answer different questions, so a result averaged across them answers none of them — and `flat` in particular is the regression guard, whose whole value is destroyed by a creased receipt being counted among it. A prefix nobody recognises lands in a visible `unsorted` group rather than in the nearest bucket. |
| Results are counts, never percentages | Nine photos, nine tick marks | A percentage over nine photos implies a precision that is not there, and invites treating a one-receipt difference as a finding. |
| A receipt with no date on it is a better test than one with a date | A "no date on it" tick, scored strictly, rather than an empty box | The fixture receipts turned out not to print dates, which looked like a gap and is the opposite. `findDate` takes the first date-shaped run of digits on any line, so on a receipt that *has* a date, an invented one is hidden behind the correct answer. On a receipt with none, there is nothing to hide behind: any date at all is an invention, and inventions are the failure this project has had four times. An empty box could only be skipped; the tick turns the same photo into an assertion. |
| Measured, not assumed: `findDate` will invent a date, from one shape | `05-06-24` in a product code becomes 5 June 2024 | Probed directly against `normalizeReceipt` rather than argued about. A phone number, a till number, a business id, a card number and a dotted price were all correctly refused — `NUMERIC_DATE` needs three groups with two separators, which most receipt debris does not have. So the risk is narrow and real rather than pervasive, and worth saying at that size. Not fixed here: on a real receipt `05-06-24` genuinely could be the date, so tightening it is a design decision about evidence, not a bug fix. |
| Only the total gates a run; every other field is scored when it is known | Blank means unscored, not failed | Requiring all three fields blocked ten photos on a field that was not on the paper. An unrecorded field counted as a failed read would also make a preparation look worse the less anyone bothered to label, which is a scoreboard that rewards laziness with a better number. `unscored` is kept apart from `missing` for that reason: one is "nothing to compare against", the other is "there was, and nothing came back". |
| The engine's page segmentation is measured, not assumed | `SINGLE_BLOCK` added to the bench as a second axis beside the preparations | `tesseract.ts` sets no mode, so the engine runs full automatic layout analysis — which includes splitting a page into columns. A till receipt is two columns with a wide gutter, exactly the shape that analysis separates, and if it does then `YHTEENSÄ` and `13,62` arrive on different text lines and `findTotal` cannot match a keyword and an amount on one line. That is a third failure beside "the pixels lost it" and "the parser chose wrongly", it is invisible in a confidence score, and neither local binarisation nor perspective correction would move it. One extra pass per photo settles which of the three is happening, which is cheaper than building the wrong fix. |
| The README's central claim was measured and withdrawn | "It works on a flat, well-lit receipt photographed straight on" — zero correct totals from three such receipts | The claim came from two photos and no scoring; ten photos with hand-typed totals falsified it. Corrected in place, in both the places it appeared, saying what was measured and that the cause is not yet established — the same treatment the receipt-scanning reversal got, and for the same reason: a claim that quietly changes is worth less than one that says it changed. |
| The three checks are ranked, not tallied | A disagreeing card line contradicts; a disagreeing item sum mostly does not | "Any disagreement wins over any agreement" assumed the checks were equally reliable. The VAT and card lines are each *one* line, large and isolated, about as readable as the total itself; the item sum aggregates many small lines and is right only if all of them read. So on a receipt whose total is perfect, a disagreeing sum is the *likeliest* outcome — measured: five of ten real receipts had the total read exactly right and contradicted anyway, and a contradicted total is blanked, so the feature threw away five correct answers it already had. Agreement from the item sum stays strong: a six-term sum matching to the cent is not a coincidence that happens. |
| An item sum that exceeds the total still contradicts | OCR drops amounts; it does not invent them | Every price in the list was read off the paper, so the receipt's true item total is at least the sum, and a total below it is too small whatever else was missed. This is the direction rule that keeps 24,90-read-as-21,90 and 24,90-read-as-4,90 caught. The old code had the asymmetry the other way round — it called a *shortfall* "always wrong" — which is right for a perfectly read receipt and exactly backwards for OCR, where a dropped price is the commonest thing that happens. |
| A shortfall is excused only by positive evidence of a missing line | `findItems` counts the amounts it saw and refused | The first attempt excused any shortfall a single misread price could explain, and the existing tests caught it letting 24,90 through as 21,90 — the precise failure the module exists to prevent. Narrowed to "this receipt printed an amount that could not be read as a line", it regressed nothing: all 307 tests passed without one being edited. A guess about what *might* have gone missing is not evidence; a count of what demonstrably did is. |
| Keywords are matched inside compound words | `KORTTIMAKSU` is *kortti* + *maksu*, and was matching neither | Found while testing the change above, and it was causing two failures at once on every Finnish card receipt: the card line went unrecognised so the total lost its free second reading, *and* it went uncounted as a non-item so it was added to the basket as a purchase — pushing the item sum past the total and turning the one corroborating line into a contradicting one. `kaikkiyhteensa` had already been hand-added to `TOTAL_WORDS` to paper over the same gap, which is the whitelist this file's own header warns about twice. Looking inside the word removed the list rather than lengthening it, and that entry is gone. Compounds start at five characters, because "sum", "net", "vat" and "card" occur inside ordinary words. |
| The relaxation is on trial, with the gate agreed in advance | One clearly marked branch, reverted to `contradicted` if the re-run shows a silent wrong answer | It is the only change that can let a wrong total through unflagged, and the known hole is a receipt that has *both* a misread total and a mangled item price — the shortfall then looks explained when it is not. Naming the gate before seeing the numbers is the point: a measure agreed afterwards is one chosen to be passed. |
| The card line refuses a bare integer; the total still accepts one | A card charge is always printed with cents, so an integer there is debris | Found by re-running OCR over the ten fixtures in Node and tracing what `findTotal` matched: a damaged `Korttimaksu < 3` handed back 3,00 as the amount paid and destroyed a total of 13,62 that had been read perfectly. Every misparse of that line seen on real receipts was a bare integer — 3, 4, 7, 1988 — while the printed value had cents and was sitting on the same line. The total must keep accepting integers, because a total that lost its comma is exactly that shape and catching one is the point of the checks; the card line has no such excuse. `findItems` already applies this rule to prices for the same reason. |
| `lastAmount` on a payment line takes the last amount *with cents* | Not simply the last amount | The same fix, and it also catches the other half: OCR puts the VAT class letter after the amount and sometimes reads it as a digit, so trailing debris wins precisely because it comes last. Filtering to cent-carrying tokens picks the printed value out of `13,62 8` without guessing at anything. |
| A card line reading `2490` against a total of `24,90` is now unverified, not contradicted | The damaged line is the card line, not the total | This was an existing test asserting `contradicted`, and it was throwing away a correct total because the corroborating line was misread. Ignoring an unreadable card value costs a corroboration that was never real; trusting it destroys a total that was. The genuine case it was meant to cover — two readings that differ and are both legible — is now its own test with `29,40`. |
| A `3 x 0,45` line is a breakdown, not a purchase | Excluded from the items, and not counted as a missing amount either | It explains the line above it and its money is already there, so counting it counts it twice — which pushed a real receipt's item sum to 14,44 against a correct total of 13,62. Since a sum *exceeding* the total is treated as strong evidence the total is too small, the double count was being fed into the check that trusts it most. Anchored at the start of the line, so an item stating its quantity inline is left alone. Measured: 7 of 20 runs affected, 3,15 of double-counted money removed — and no verdict changed, because the same receipts carry other OCR damage that keeps the sum wrong. Right, and not sufficient. |
| A sum above the total is strong evidence, with one known hole | OCR drops amounts, but it can still inflate one it did read | The rule assumes a read sum is a lower bound on the receipt's real total. That holds for omission and fails for a misread digit — `1,35` read as `1,95` inflates the sum without any line being invented. Seen on `flat-01`, where the sum stays above the total after the double count is removed. Left alone deliberately: tightening it is what keeps 24,90-read-as-21,90 caught, and trading a false contradiction for a silent wrong answer is the wrong direction. |
| The README was corrected twice, in opposite directions, and both are on the page | Worked on flat receipts → zero → eleven of twenty | The first number came from two photos and no scoring; the second from ten photos and hand-typed totals; the third from fixing what the second one revealed. Publishing the *worse* number is what makes the better one worth anything — a document that only ever improves is a document nobody measured. The middle correction stays in the text rather than being replaced by the good news. |

...
| A compound question is refused, not half-answered | Both parts named, and neither guessed at | Both halves are answerable on their own; it is the pair that one query cannot return. Answering the first and staying quiet about the second is the same failure as answering "why did I spend so much on food" with a figure — it looks like an answer and it is not one. Answering both would mean composing two queries and two sentences, which is a larger grammar than this feature has; naming both and asking for one at a time is the honest version of the same refusal. |

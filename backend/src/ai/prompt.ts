...
export function summarySystemPrompt(): string {
  return [
    ...
    "say there is too little before it for a comparison to mean anything. Never",
    "compute a percentage against a baseline the field has not called usable,",
    "and never leave the comparison out silently — a missing sentence reads as",
    "spending that did not change.",
